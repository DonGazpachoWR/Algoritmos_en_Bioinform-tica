...
smith_waterman = function(seq1, seq2, gap, gap_extension, mismatch, match){
        ...
        for (i in 2:(len1 + 1)){# for every (initially zero) row
                for (j in 2:(len2 + 1)){# for every (initially zero) col
                        hor = ifelse(D[i, j - 1] == "dia", # horizontal move = ifelse(previous movement for seq1, gap, gap extension)
                                     M[i, j - 1] + gap,
                                     M[i, j - 1] + gap_extension)
                        ver = ifelse(D[i - 1, j] == "dia", # vertical move = ifelse(previous movement for seq2, gap, gap extension)
                                     M[i - 1, j] + gap, 
                                     M[i - 1, j] + gap_extension)
...
solution_s <- smith_waterman("CTCGTTTCAGAAC", "CGACTCGTTAGAT", gap = -3, gap_extension = -2, mismatch = -4, match = 5)
cat("\nAligned sequences\n")
